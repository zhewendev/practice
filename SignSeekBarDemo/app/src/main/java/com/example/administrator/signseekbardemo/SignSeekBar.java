package com.example.administrator.signseekbardemo;
...
import static com.example.administrator.signseekbardemo.SignSeekBar.TextPosition.BELOW_SECTION_MARK;
import static com.example.administrator.signseekbardemo.SignSeekBar.TextPosition.BOTTOM_SIDES;
import static com.example.administrator.signseekbardemo.SignSeekBar.TextPosition.SIDES;
...
public class SignSeekBar extends View {
    ...
    private void init(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.SignSeekBar, defStyleAttr, 0);
        ...
        mThumbOuterRadius = a.getDimensionPixelSize(R.styleable.SignSeekBar_ssb_thumb_radius, mSecondTrackSize * 2);
    ...
    private void initConfigByPriority() {
        ...
        if (mThumbOuterRadius <= mSecondTrackSize) {
            mThumbOuterRadius = mSecondTrackSize * 2;
        }
        if (mSectionCount <= 0) {
            mSectionCount = 10;
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = mThumbOuterRadius * 2; // 默认高度为拖动时thumb圆的直径
        if (isShowThumbText) {
